...
export default function Notifications(){
    ...
    const getNotifications = () => {
        fetch(
            "http://localhost/sally-hostel/php/processes/HostelOwner/LoadUserNotifications.php", {
                method: "POST",
                headers: {
        ...
        if(notifications.length > 0){
                ...
                <>
                    {
                        notifications.map(
                            (notification_data, index) => (
                                <div key = {index} className = {"rounded-2x mb-3 border cursor-pointer shadow" + (
                                    ...
                                    <div>
                                        <div className = "flex-h p-3">
                                            <div className = "pl-3">
                                                <div title = {notification_data.id} className = {(
                                                    (doubleLine)
                                                    ? "double-line"
                                                    : ""
                                                )} onClick = {
                                                    (e) => {
                                                        setDoubleLine(!doubleLine)
            ...
            return (
                <div className = "col-12 p-5 shadow border rounded-2x bg-light text-c text-muted half-bold">
                    You don't any notification yet. Important information will be diplayed here.
                </div>
            )
        }
    }
