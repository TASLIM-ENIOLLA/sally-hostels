...
const MostRatedHostels = () => {
    ...
    React.useEffect(
        () => {
            ...
            fetch(
                "http://localhost/sally-hostel/php/processes/Student/LoadMostRatedHostels.php", {
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json',
                    },
    ...
    mostRatedHostelsData = mostRatedHostelsData.map(
        (each, key) =>
        <div key = {key} className = "p-3 col-md-3 mx-3 bg-white shadow rounded-2x" style = {{
            minWidth: "250px"
        }}>
            <Link to = {"/student/view-hostel/?hostel_id=" + each.id} className = "hostel-image d-block w-100 rounded-2x" style = {{
                minHeight: "150px",
    ...
    //     </div>
    // )
    return mostRatedHostelsData

}
export default MostRatedHostels
